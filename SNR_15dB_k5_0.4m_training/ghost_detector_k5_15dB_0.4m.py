"""
SNR 15dB + k=5 + 거리 0.4m 기준 고스트 타겟 탐지 시스템
균형잡힌 중간 지점 설정
"""
import torch
...
class BalancedGhostDetectorDataset:
    """k=5, SNR=15dB, 거리=0.4m 균형잡힌 데이터셋"""
    
    def __init__(self, 
                 radar_data_path: str,
                 lidar_data_path: str,
                 distance_threshold: float = 0.4,  # 중간 지점
                 snr_threshold: float = 15.0,      # 중간 지점
                 k: int = 5,                       # 중간 지점
    ...
    def _process_data(self):
        """데이터 처리 및 라벨링"""
        print("균형잡힌 기준으로 SNR + 거리 조합 라벨링 중...")
        ...
        for i, radar_frame in enumerate(self.radar_frames):
            ...
            # 균형잡힌 기준으로 라벨링
            labels = self._label_points_balanced(radar_frame, lidar_frame)
        ...
        print(f"\n=== 균형잡힌 기준 데이터셋 통계 ===")
        print(f"📏 거리 임계값: {self.distance_threshold}m (중간 지점)")
        print(f"📡 SNR 임계값: {self.snr_threshold}dB (중간 지점)")
        print(f"🔗 k-NN 연결: {self.k}개 (중간 지점)")
    ...
    def _label_points_balanced(self, radar_points, lidar_frame):
        """균형잡힌 SNR + 거리 조합 라벨링"""
        if not lidar_frame:
...
def create_training_visualization(dataset, save_path="balanced_training_analysis.png"):
    """균형잡힌 학습 데이터 시각화"""
    print("균형잡힌 학습 데이터 시각화 생성 중...")
    ...
    for i, radar_frame in enumerate(dataset.radar_frames[:100]):  # 처음 100프레임만
        ...
        # 레이더 포인트와 라벨
        labels = dataset._label_points_balanced(radar_frame, lidar_frame)
    ...
    ax2.set_title('2. Balanced Labeling (k=5, 15dB, 0.4m)', fontsize=14)
    ax2.set_xlabel('X (m)')
    ax2.set_ylabel('Y (m)')
    ax2.legend()
    ...
    ax3.hist(valid_distances, bins=50, alpha=0.7, color='orange', edgecolor='black')
    ax3.axvline(x=0.4, color='orange', linestyle='--', linewidth=2, label='0.4m threshold')
    ax3.axvline(x=0.5, color='blue', linestyle='--', linewidth=2, label='0.5m (previous)')
    ax3.axvline(x=0.3, color='red', linestyle='--', linewidth=2, label='0.3m (strict)')
    ...
    stats_text = f"""
    📊 Balanced Training Data Analysis
    
    🎯 Balanced Criteria:
    • Distance threshold: 0.4m (middle point)
    • k-NN connections: 5 (middle point)
    • SNR threshold: 15.0 dB (middle point)
    ...
    • Moderate neighbor influence
    • Practical distance threshold
    • Domain-adapted SNR threshold
    """
    ...
    print(f"Balanced training visualization saved: {save_path}")
    
    return len(all_real_x), len(all_ghost_x)

def train_balanced_ghost_detector():
    """균형잡힌 고스트 탐지기 학습"""
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"🚀 균형잡힌 GPU 학습 시작! 장치: {device}")
    print(f"📏 거리 임계값: 0.4m (중간 지점)")
    print(f"📡 SNR 임계값: 15dB (도메인 적응)")
    print(f"🔗 k-NN 연결: 5개 (균형잡힌 이웃 영향)")
    
    # 균형잡힌 데이터셋
    dataset = BalancedGhostDetectorDataset(
        radar_data_path="RadarMap_v2.txt",
        lidar_data_path="LiDARMap_v2.txt",
        distance_threshold=0.4,  # 중간 지점
        snr_threshold=15.0,      # 도메인 적응
        k=5                      # 균형잡힌 이웃
    )
    ...
    for epoch in range(50):
        ...
        if (epoch + 1) % 10 == 0:
            ...
            if val_acc > best_val_acc:
                ...
                torch.save(model.state_dict(), 'ghost_detector_k5_15dB_0.4m.pth')
            
            print(f"Epoch {epoch+1}/50 | Train Loss: {total_loss/len(train_loader):.4f} | Val Acc: {val_acc:.2f}% | Best: {best_val_acc:.2f}%")
    
    print(f"🎉 균형잡힌 학습 완료! 최고 정확도: {best_val_acc:.2f}%")
    return best_val_acc

if __name__ == "__main__":
    train_balanced_ghost_detector()
