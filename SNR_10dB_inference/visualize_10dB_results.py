...
class VisualizationInference:
    ...
    def run_inference(self):
        ...
        for i, radar_frame in enumerate(self.radar_frames):
            ...
            # 그래프 데이터 생성
            graph_data = create_graph_data(radar_frame, k=8)
        ...
        print(f"Inference completed for {len(self.results)} frames")
    
    def visualize_all_frames(self, save_path: str = "SNR_10dB_all_frames.png"):
        """모든 프레임을 하나의 그림에 시각화"""
        print("Creating visualization for all frames...")
        ...
        ax2.set_title('2. Model Predictions (SNR=10dB)', fontsize=14)
        ax2.set_xlabel('X (m)')
        ax2.set_ylabel('Y (m)')
        ax2.legend()
        ...
        ax3.set_title('3. Prediction Probabilities', fontsize=14)
        ax3.set_xlabel('X (m)')
        ax3.set_ylabel('Y (m)')
        ...
        print(f"All frames visualization saved: {save_path}")
        
        return len(all_real_x), len(all_ghost_x), np.mean(all_probabilities)
    
    def visualize_radar_focused(self, save_path: str = "SNR_10dB_radar_focused.png", margin: float = 5.0):
        """레이더 포인트 기준으로 확대된 시각화"""
        print("Creating radar-focused visualization...")
        ...
        ax1.set_title('1. Radar-Focused View: Original Data', fontsize=14)
        ax1.set_xlabel('X (m)')
        ax1.set_ylabel('Y (m)')
        ax1.legend()
        ax1.grid(True, alpha=0.3)
        ...
        ax2.set_title('2. Radar-Focused: Model Predictions', fontsize=14)
        ax2.set_xlabel('X (m)')
        ax2.set_ylabel('Y (m)')
        ax2.legend()
        ax2.grid(True, alpha=0.3)
        ...
        ax3.set_title('3. Radar-Focused: Prediction Probabilities', fontsize=14)
        ax3.set_xlabel('X (m)')
        ax3.set_ylabel('Y (m)')
        ax3.grid(True, alpha=0.3)
        plt.colorbar(scatter3, ax=ax3, label='Real Target Probability')
        ...
        🎯 Benefits:
        • Clearer radar pattern visibility
        • Reduced LiDAR noise
        • Better target discrimination
        
        🚀 Model: SNR 10dB threshold
        """
        ...
        plt.tight_layout()
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        print(f"Radar-focused visualization saved: {save_path}")

def main():
    # 추론 실행
    inferencer = VisualizationInference(
    ...
    print("   - SNR_10dB_all_frames.png")
    print("   - SNR_10dB_radar_focused.png")
