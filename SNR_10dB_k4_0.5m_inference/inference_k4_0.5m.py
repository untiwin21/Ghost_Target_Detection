"""
k=4, 거리=0.5m 기준 최적화된 모델로 추론 및 시각화
"""
import torch
import numpy as np
...
class OptimizedInference:
    def __init__(self, model_path: str, device: str = 'auto'):
        ...
        print(f"Model criteria: k=4, distance=0.5m, SNR=10dB")
    ...
    def run_inference(self):
        ...
        print(f"Optimized inference completed for {len(self.results)} frames")
    
    def visualize_all_frames(self, save_path: str = "optimized_all_frames.png"):
        """모든 프레임을 하나의 그림에 시각화"""
        print("Creating optimized visualization for all frames...")
        ...
        ax2.set_title('2. Optimized Model Predictions (k=4, 0.5m)', fontsize=14)
        ax2.set_xlabel('X (m)')
        ax2.set_ylabel('Y (m)')
        ax2.legend()
        ...
        ax3.set_title('3. Optimized Prediction Probabilities', fontsize=14)
        ax3.set_xlabel('X (m)')
        ax3.set_ylabel('Y (m)')
        ...
        ax4.axis('off')
        stats_text = f"""
        📊 Optimized Model Results (k=4, 0.5m)
        ...
        🎯 Optimized Criteria:
        • Distance threshold: 0.5m (balanced)
        ...
        print(f"Optimized all frames visualization saved: {save_path}")
        
        return len(all_real_x), len(all_ghost_x), np.mean(all_probabilities)
    
    def visualize_radar_focused(self, save_path: str = "optimized_radar_focused.png", margin: float = 5.0):
        """레이더 포인트 기준으로 확대된 최적화된 시각화"""
        print("Creating optimized radar-focused visualization...")
        ...
        ax1.set_title('1. Optimized Radar-Focused: Original Data', fontsize=14)
        ax1.set_xlabel('X (m)')
        ax1.set_ylabel('Y (m)')
        ax1.legend()
        ax1.grid(True, alpha=0.3)
        ...
        ax2.set_title('2. Optimized Radar-Focused: Model Predictions', fontsize=14)
        ax2.set_xlabel('X (m)')
        ax2.set_ylabel('Y (m)')
        ax2.legend()
        ax2.grid(True, alpha=0.3)
        ...
        ax3.set_title('3. Optimized Radar-Focused: Prediction Probabilities', fontsize=14)
        ax3.set_xlabel('X (m)')
        ax3.set_ylabel('Y (m)')
        ax3.grid(True, alpha=0.3)
        plt.colorbar(scatter3, ax=ax3, label='Real Target Probability')
        ...
        plt.tight_layout()
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        print(f"Optimized radar-focused visualization saved: {save_path}")

def main():
    # 추론 실행
    inferencer = OptimizedInference(
        model_path="ghost_detector_k4_0.5m.pth",
        device='auto'
    )
    ...
    print("   - optimized_all_frames.png")
    print("   - optimized_radar_focused.png")
