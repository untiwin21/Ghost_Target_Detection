...
class StrictInference:
    def __init__(self, model_path: str, device: str = 'auto'):
        ...
        print(f"Strict model loaded from {model_path}")
        print(f"Model criteria: k=4, distance=0.3m, SNR=10dB")
    ...
    def run_inference(self):
        ...
        print(f"Strict inference completed for {len(self.results)} frames")
    
    def visualize_all_frames(self, save_path: str = "strict_all_frames.png"):
        """모든 프레임을 하나의 그림에 시각화"""
        print("Creating strict visualization for all frames...")
        ...
        ax2.set_title('2. Strict Model Predictions (k=4, 0.3m)', fontsize=14)
        ax2.set_xlabel('X (m)')
        ax2.set_ylabel('Y (m)')
        ax2.legend()
        ...
        ax3.set_title('3. Strict Prediction Probabilities', fontsize=14)
        ax3.set_xlabel('X (m)')
        ax3.set_ylabel('Y (m)')
        ...
        print(f"Strict all frames visualization saved: {save_path}")
        
        return len(all_real_x), len(all_ghost_x), np.mean(all_probabilities)
    
    def visualize_radar_focused(self, save_path: str = "strict_radar_focused.png", margin: float = 5.0):
        """레이더 포인트 기준으로 확대된 엄격한 시각화"""
        print("Creating strict radar-focused visualization...")
        ...
        ax1.set_title('1. Strict Radar-Focused: Original Data', fontsize=14)
        ax1.set_xlabel('X (m)')
        ax1.set_ylabel('Y (m)')
        ax1.legend()
        ax1.grid(True, alpha=0.3)
        ...
        ax2.set_title('2. Strict Radar-Focused: Model Predictions', fontsize=14)
        ax2.set_xlabel('X (m)')
        ax2.set_ylabel('Y (m)')
        ax2.legend()
        ax2.grid(True, alpha=0.3)
        ...
        ax3.set_title('3. Strict Radar-Focused: Prediction Probabilities', fontsize=14)
        ax3.set_xlabel('X (m)')
        ax3.set_ylabel('Y (m)')
        ax3.grid(True, alpha=0.3)
        plt.colorbar(scatter3, ax=ax3, label='Real Target Probability')
        ...
        • Less false positive real targets
        • More conservative predictions
        
        🚀 Model: Strict k=4, 0.3m
        """
        ...
        plt.tight_layout()
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        print(f"Strict radar-focused visualization saved: {save_path}")

def main():
    # 추론 실행
    inferencer = StrictInference(
        model_path="ghost_detector_k4_0.3m.pth",
        device='auto'
    )
    ...
    # 시각화 생성
    print("\n" + "="*50)
    print("🎨 Creating strict visualizations...")
