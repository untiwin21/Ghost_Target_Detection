#!/usr/bin/env python3
"""
레이더 고스트 타겟 탐지 시스템 - SNR 10dB + k=6 + 거리 0.4m 학습
...
def create_labels_snr_distance(radar_frame: RadarFrame, lidar_frame: LiDARFrame, 
                              snr_threshold: float = 10.0, distance_threshold: float = 0.4) -> List[int]:
    """SNR + 거리 조합 기반 라벨 생성 (10dB + 0.4m)"""
    labels = []
    ...
    def _create_graphs(self):
        ...
        for timestamp in self.radar_frames:
            if timestamp in self.lidar_frames:
                ...
                # 라벨 생성 (SNR 10dB + 거리 0.4m)
                frame_labels = create_labels_snr_distance(radar_frame, lidar_frame, 
                                                        snr_threshold=10.0, distance_threshold=0.4)
...
def train_model():
    ...
    print("⚙️  설정: SNR ≥ 10dB + 거리 ≤ 0.4m + k=6")
    ...
    for epoch in range(50):
        ...
        if val_acc > best_val_acc:
            ...
            patience_counter = 0
            # 최고 모델 저장
            torch.save(model.state_dict(), 'ghost_detector_k6_10dB_0.4m.pth')
        else:
            patience_counter += 1
...
def create_training_visualization(train_losses, val_losses, train_accuracies, val_accuracies, dataset):
    ...
    ax1.set_title('Training and Validation Loss (k=6, SNR≥10dB, dist≤0.4m)', fontsize=12, fontweight='bold')
    ...
    ax3.set_title('Target Distribution (k=6, SNR≥10dB)', fontsize=12, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig('k6_10dB_training_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print("📊 학습 분석 결과가 'k6_10dB_training_analysis.png'에 저장되었습니다.")
