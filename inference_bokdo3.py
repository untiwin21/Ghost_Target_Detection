"""
bokdo3_v6 데이터로 개선된 모델 추론 및 시각화
"""
import torch
import numpy as np
...
class Bokdo3GhostDetectorInference:
    """bokdo3 데이터로 추론하는 클래스"""
    ...
    def __init__(self, 
        ...
        print(f"🚀 Bokdo3 추론 장치: {self.device}")
    ...
    def load_data(self):
        ...
        if not os.path.exists(self.radar_data_path):
            print(f"⚠️ 레이더 파일이 없습니다: {self.radar_data_path}")
            return []
        ...
        if not os.path.exists(self.lidar_data_path):
            print(f"⚠️ LiDAR 파일이 없습니다: {self.lidar_data_path}")
            return []
    ...
    def visualize_bokdo3_results(self, num_frames=50):
        """bokdo3 결과 시각화"""
        print(f"🎨 Bokdo3 결과 시각화 중... ({num_frames}개 프레임)")
        ...
        fig.suptitle(f'Bokdo3 Ghost Detection Results (SNR ≥ {self.snr_threshold}dB)', 
        ...
        ax1.set_title('Bokdo3 Original Data: LiDAR + Radar')
        ax1.set_xlabel('X (m)')
        ax1.set_ylabel('Y (m)')
        ax1.legend()
        ...
        ax2.set_title(f'Ground Truth (SNR ≥ {self.snr_threshold}dB)')
        ax2.set_xlabel('X (m)')
        ax2.set_ylabel('Y (m)')
        ax2.legend()
        ...
        ax5.set_title(f'Bokdo3 Real Targets Only (n={np.sum(predicted_real):,})')
        ax5.set_xlabel('X (m)')
        ax5.set_ylabel('Y (m)')
        ax5.legend()
        ...
        # SNR 통계
        snr_mean = np.mean(radar_snrs)
        snr_std = np.std(radar_snrs)
        snr_min = np.min(radar_snrs)
        snr_max = np.max(radar_snrs)
        
        stats_text = f"""
Bokdo3 Dataset Performance

Dataset Statistics:
• Total Points: {len(all_predictions):,}
• Frames Processed: {frames_to_process}
• SNR Range: {snr_min:.1f} - {snr_max:.1f} dB
• SNR Mean: {snr_mean:.1f} ± {snr_std:.1f} dB
...
Results:
• Real Targets: {np.sum(predicted_real):,} ({real_retention:.1f}%)
• Ghost Targets: {np.sum(predicted_ghost):,}
• SNR Threshold: {self.snr_threshold} dB
...
• False Negative: {fn:,}
        ...
        plt.tight_layout()
        plt.savefig('bokdo3_inference_results.png', dpi=300, bbox_inches='tight')
        plt.show()
...
def main():
    """메인 실행 함수"""
    print("🚀 Bokdo3 데이터로 개선된 모델 추론 시작!")
    
    # 추론 실행
    inference = Bokdo3GhostDetectorInference()
    ...
    # 시각화
    results = inference.visualize_bokdo3_results(num_frames=100)
    ...
    if results:
        ...
        print(f"   - SNR 평균: {results['snr_stats']['mean']:.1f} ± {results['snr_stats']['std']:.1f} dB")
        print(f"💾 결과 저장: bokdo3_inference_results.png")
    else:
        print("❌ 추론 실패!")
