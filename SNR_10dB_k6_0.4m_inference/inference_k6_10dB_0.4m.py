#!/usr/bin/env python3
"""
레이더 고스트 타겟 탐지 추론 시스템 - SNR 10dB + k=6 + 거리 0.4m
...
def run_inference():
    ...
    print("⚙️  설정: SNR ≥ 10dB + k=6 + 거리 0.4m")
    
    # 모델 로드
    model = HybridGhostGNN(input_dim=6, hidden_dim=128, num_layers=3, dropout=0.1).to(device)
    
    # 학습된 가중치 로드
    if os.path.exists('ghost_detector_k6_10dB_0.4m.pth'):
        model.load_state_dict(torch.load('ghost_detector_k6_10dB_0.4m.pth', map_location=device))
        print("✅ 학습된 모델 로드 완료")
    else:
...
def create_visualizations(radar_points, lidar_points, predictions, probabilities):
    ...
    plt.tight_layout()
    plt.savefig('k6_10dB_all_frames.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig('k6_10dB_radar_focused.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print("📊 시각화 완료:")
    print("  - k6_10dB_all_frames.png: 전체 뷰")
    print("  - k6_10dB_radar_focused.png: 레이더 중심 확대 뷰")
