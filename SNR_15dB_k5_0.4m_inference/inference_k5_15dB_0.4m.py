"""
k=5, SNR=15dB, 거리=0.4m 기준 균형잡힌 모델로 추론 및 시각화
"""
import torch
import numpy as np
...
class BalancedInference:
    def __init__(self, model_path: str, device: str = 'auto'):
        ...
        print(f"Balanced model loaded from {model_path}")
        print(f"Model criteria: k=5, SNR=15dB, distance=0.4m")
    ...
    def run_inference(self):
        ...
        for i, radar_frame in enumerate(self.radar_frames):
            ...
            # 그래프 데이터 생성 (k=5 사용)
            graph_data = create_graph_data(radar_frame, k=5)
        ...
        print(f"Balanced inference completed for {len(self.results)} frames")
    
    def visualize_all_frames(self, save_path: str = "balanced_all_frames.png"):
        """모든 프레임을 하나의 그림에 시각화"""
        print("Creating balanced visualization for all frames...")
        ...
        ax2.set_title('2. Balanced Model Predictions (k=5, 15dB, 0.4m)', fontsize=14)
        ax2.set_xlabel('X (m)')
        ax2.set_ylabel('Y (m)')
        ax2.legend()
        ...
        ax3.set_title('3. Balanced Prediction Probabilities', fontsize=14)
        ax3.set_xlabel('X (m)')
        ax3.set_ylabel('Y (m)')
        ...
        ax4.axis('off')
        stats_text = f"""
        📊 Balanced Model Results (k=5, 15dB, 0.4m)
        ...
        🎯 Balanced Criteria:
        • Distance threshold: 0.4m (middle point)
        • k-NN connections: 5 (balanced influence)
        • SNR threshold: 15.0 dB (domain-adapted)
        
        💡 Domain Adaptation:
        • Training avg SNR: 25.3dB
        • Inference avg SNR: 20.9dB
        • Adapted threshold: 15dB (vs 20dB)
        ...
        print(f"Balanced all frames visualization saved: {save_path}")
        
        return len(all_real_x), len(all_ghost_x), np.mean(all_probabilities)
    
    def visualize_radar_focused(self, save_path: str = "balanced_radar_focused.png", margin: float = 5.0):
        """레이더 포인트 기준으로 확대된 균형잡힌 시각화"""
        print("Creating balanced radar-focused visualization...")
        ...
        ax1.set_title('1. Balanced Radar-Focused: Original Data', fontsize=14)
        ax1.set_xlabel('X (m)')
        ax1.set_ylabel('Y (m)')
        ax1.legend()
        ax1.grid(True, alpha=0.3)
        ...
        ax2.set_title('2. Balanced Radar-Focused: Model Predictions', fontsize=14)
        ax2.set_xlabel('X (m)')
        ax2.set_ylabel('Y (m)')
        ax2.legend()
        ax2.grid(True, alpha=0.3)
        ...
        ax3.set_title('3. Balanced Radar-Focused: Prediction Probabilities', fontsize=14)
        ax3.set_xlabel('X (m)')
        ax3.set_ylabel('Y (m)')
        ax3.grid(True, alpha=0.3)
        plt.colorbar(scatter3, ax=ax3, label='Real Target Probability')
        ...
        🎯 Balanced Criteria:
        • Distance threshold: 0.4m (middle)
        • k-NN connections: 5 (balanced)
        • SNR threshold: 15dB (domain-adapted)
        
        💡 Domain Adaptation Benefits:
        • Adapted to bokdo3 SNR distribution
        • LiDAR-adjacent radars preserved
        • Balanced precision/recall trade-off
        • Reduced domain gap impact
        
        🚀 Model: Balanced k=5, 15dB, 0.4m
        """
        ...
        plt.tight_layout()
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        print(f"Balanced radar-focused visualization saved: {save_path}")

def main():
    # 추론 실행
    inferencer = BalancedInference(
        model_path="ghost_detector_k5_15dB_0.4m.pth",
        device='auto'
    )
    ...
    # 시각화 생성
    print("\n" + "="*50)
    print("🎨 Creating balanced visualizations...")
    ...
    print("   - balanced_all_frames.png")
    print("   - balanced_radar_focused.png")
